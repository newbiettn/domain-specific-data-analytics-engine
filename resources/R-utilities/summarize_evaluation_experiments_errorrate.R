...
################################################################################
# Load dataset
dt <- read.csv("evaluation_experiment_gmeans_errorrate.csv")
# dt <- read.csv("evaluation_experiment_kmeans_errorrate.csv")
...
ggplot(melt(performance.by.rank)) +
  ...
  xlab("Workflow") +
  ylab("Error Rate") +
  ggtitle(paste0(number.of.workflows, " workflows", " - Error Rate (", clustering.technique, ")")) +
  theme_bw()
filename = paste0("evaluation_experiment-", clustering.technique, "_errorrate-direct_comparison_size_", number.of.workflows, ".png")
ggsave(filename)
################################################################################
...
create.comparison.window.1 <- function(size, size.list, performance.by.rank){
  ...
  best.of.window.1 <- apply(window.1, 1, min)
  best.of.window.2 <- apply(window.2, 1, min)
  fraction <- round((best.of.window.2/best.of.window.1)*100, digits = 4)
  return (fraction)
}
...
ggplot(melt(window.performance)) +
  ...
  xlab("Case") +
  ylab("Error Rate") +
  ggtitle("Comparison windows by error rate") +
  theme_bw() +
  theme(legend.position="bottom", 
        legend.background = element_rect(fill="gray90", size=1, linetype="dotted")) +
  scale_fill_manual("Case", values=wes_palette(n=3, name="Moonrise3"))
...
filename = paste0("evaluation_experiment-", clustering.technique, "_errorrate-fraction_size_", number.of.workflows, ".png")
ggsave(filename)
################################################################################
...
filename <-  paste0("evaluation_experiment-", clustering.technique, "_errorrate-windows_size_", number.of.workflows, ".png")
ggsave(filename)
