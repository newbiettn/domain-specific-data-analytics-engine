...
dt <- dt %>%
  group_by(DATASET) %>%
  arrange(DATASET, MEANERRORRATE) %>%
  mutate(rankByErrorRate = rank(MEANERRORRATE, ties.method = "min")) %>%
  as.data.frame()

dt <- dt %>%
  group_by(DATASET, rankByErrorRate) %>%
  arrange(DATASET, rankByErrorRate, MEANTIMEELAPSED) %>%
...
dt <- dt %>%
  group_by(DATASET, rankByErrorRate, rankByTime) %>%
  arrange(DATASET, rankByErrorRate, rankByTime) %>%
  mutate(rank = myrank(rankByErrorRate, rankByTime)) %>%
  as.data.frame()
