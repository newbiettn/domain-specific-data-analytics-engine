 ...
 */
public class TargetNodeSkin extends CustomFlowNodeSkin {

    public TargetNodeSkin(FXSkinFactory skinFactory,
    ...
    protected Node createView() {
        FXMLLoader fxmlLoader = new FXMLLoader(this.getClass()
                .getClassLoader().getResource("fxml/TargetNode.fxml"));
        try {
            fxmlLoader.load();
        } catch (IOException ex) {
            Logger.getLogger(TargetNodeSkin.class.getName()).
                    log(Level.SEVERE, null, ex);
        }
        getModel().setTitle("Target Node");
        getNode().setPrefSize(30, 80);

        TargetNodeController controller = fxmlLoader.getController();
        controller.setNode(getModel());
        controller.setTargetNodeBean((TargetNodeBean) getModel().getValueObject().getValue());
